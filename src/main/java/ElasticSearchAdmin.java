...
//                new InetSocketTransportAddress(InetAddress.getByName("s1.hadoop"), 9300));
    ...
    public void createIndexWithSettings() {
        ...
        //准备创建索引
        indices.prepareCreate("test_test2")
                //配置索引参数
                .setSettings(
                        //参数配置器
    ...
    //正文抽取项目使用，创建test_spark索引，设置type（news）的schema
    //使用ik_smart分词器
    @Test
    public void createSparkStreamingContextMappings() throws IOException {
        ...
        CreateIndexRequestBuilder prepareCreate = client.admin().indices().prepareCreate("c20_yuyan20_index");
        //管理索引（test_spark）然后关联type（news）
